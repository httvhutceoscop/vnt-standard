...
class easyFAQs
{
			...
			if ( $_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['action']) && $_POST['action'] == "post_faq" ) {
				...
				if ( isValidFAQKey() ) {
					...
					if ( !empty($_POST['the-title']) ) {
						$title = $this->get_str('QUESTION_FROM'). $_POST['the-title'];
					} else {
	...
	function displayFAQsFromQuery( $loop, $atts = array() )
	{
		...
		//do this before opening the wrappers
	...
	function outputFAQsByCategory($atts) { 
		...
		if ( $categories_accordion ) {
			$category_classes[] = 'categories_accordion';
		}

		if ( in_array( $categories_accordion, array('accordion-collapsed', 'accordion_collapsed', 'collapsed') ) ) {
			$category_classes[] = 'categories_accordion_collapsed';
		}
		
		$category_classes = apply_filters( 'easy_faqs_category_classes', $category_classes );
